...
for II in $LINUX $WIN $MACAMD $MACARM; do
	...
	# echo $os $arch $suffix;
	echo GOOS=${os} GOARCH=${arch} CGO_ENABLED=${cgo} go build -o ${THISDIR}/${BINBASENAME}/${OUTDIR}-${suffix} .
	GOOS=${os} GOARCH=${arch} CGO_ENABLED=${cgo} go build -o ${THISDIR}/${BINBASENAME}/${OUTDIR}-${suffix} .
done
